...
<div class="assets-banner-copies mt-16 ml-12">
    <title-update :title="chooseTitle + ' Banner Copy'"></title-update>
    <back-button @link-clicked="$emit('backClicked', step)"></back-button>
    <nav-steps :step="2"></nav-steps>
    <div class="mr-6">
        <div class="mb-4">
            <h1 class="section-title">Choose Your {{chooseTitle}} Banner Copy</h1>

            <div class="headline-selector mb-4 mt-4" v-show="!subheadEditable">
                <breeze-label for="banner_headline_select" class="select-label" value="Subheading"></breeze-label>
                <breeze-select id="banner_headline_select" class="inline-block w-9/12 mb-2" v-model="subhead" :options="subheadSelect" required />
            ...
            <div class="headline-edit mb-4 mt-1" v-show="subheadEditable">
                <breeze-label for="banner_headline_override" class="select-label" value="Subheading"></breeze-label>
                <breeze-input id="banner_headline_override" type="text" class="w-9/12 mb-2" v-model="subheadOverride"></breeze-input>
            ...
            <div class="cta-selector mb-4 mt-1" v-show="!ctaEditable">
                <breeze-label for="banner_cta_select" class="select-label" value="Call to Action"></breeze-label>
                <breeze-select id="banner_cta_select" class="inline-block w-9/12 mb-2" v-model="cta" :options="ctaSelect" required />
            ...
            <div class="body-text-edit mb-4 mt-1" v-show="ctaEditable">
                <breeze-label for="banner_cta_override" class="select-label" value="Call to Action"></breeze-label>
                <breeze-input id="banner_cta_override" type="text" class="w-9/12 mb-2" rows="6" v-model="ctaOverride" maxlength="26"></breeze-input>
...
export default {
    ...
    mounted() {
        document.getElementById('layout_intro_text').innerHTML = "<div>Please <b>select the copy for your " + this.chooseTitle + " Banner Image from the dropdown menus to the right</b>. Once you have selected your text options, you may use the edit button to make minor changes to the copy directly in the selection box. Pressing Cancel will reset the text to your original choice. Please keep in mind that font size and colors are not editable and there are character limits in place.</div>";

      const ps = this.$parent.preSelected;

      [{v:"subhead", p: "banner_body"}, {v:"cta", p: "banner_cta"}].forEach(x => {
        ...
        if(!this.subhead || !this.sCloud.banner_subhead.includes(this.subhead)) {
            this.subhead = this.sCloud.banner_subhead[0];
        }

        if(!this.cta || !this.$parent.copies.common[this.selector].banner_cta.includes(this.cta)) {
            this.cta = this.$parent.copies.common[this.selector].banner_cta[0];
        }
    ...
    computed: {
        ...
        subheadSelect() {
            ...
            const a = this.sCloud.banner_subhead;
            for(let k in a) {
                list.push({
                    value: a[k],
                    label: a[k],//.strip,
        ...
        ctaSelect() {
            ...
            let list = [];
            const a = this.$parent.copies.common[this.selector].banner_cta;
            for(let k in a) {
                list.push({
                    value: a[k],
                    label: a[k],
