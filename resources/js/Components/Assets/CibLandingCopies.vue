<template>
    <div class="full-page-canvas full-page-canvas w-full sm:max-w-6xl bg-white shadow-md overflow-hidden min-h-full">
        <title-update :title="chooseTitle + ' Landing Page Copy'"></title-update>
        ...
        <div class="layout-left-part pl-28 pr-20 pt-20 pb-20">
            ...
            <div class="layout-intro-text mt-10 pr-20 mb-10">
                <div>Please <b>select the copy for your {{chooseTitle}} Landing Copy from the dropdown menus below</b>. Once you have selected your text options, you may use the edit button to make minor changes to the copy directly in the selection box. Pressing Cancel will reset the text to your original choice. Please keep in mind that font size and colors are not editable and there are character limits in place.</div>
            </div>

            <div class="selects pb-16">
                <div class="mb-4">
                    <h1 class="section-title">Choose Your {{chooseTitle}} Landing Copy</h1>
                    <div class="headline-selector mb-4 mt-4" v-show="!subheadEditable">
                        <breeze-label for="headline_select" class="select-label" value="Subheading"></breeze-label>
                        <breeze-select id="headline_select" class="inline-block w-9/12 mb-2" v-model="subhead" :options="subheadSelect" required />
                        <breeze-button type="button" class="ml-2 edit-button" @click="editSubhead" :disabled="!subhead">Edit</breeze-button>
                    </div>

                    <div class="headline-edit mb-4 mt-4" v-show="subheadEditable">
                        <breeze-label for="headline_override" class="select-label" value="Subheading"></breeze-label>
                        <breeze-input id="headline_override" type="text" class="w-9/12 mb-2" v-model="subheadOverride"></breeze-input>
                    ...
                    <div class="intro-selector mb-4 mt-1" v-show="!introEditable">
                        <breeze-label for="intro_select" class="select-label" value="Intro"></breeze-label>
                    ...
                    <div class="intro mb-4 mt-1" v-show="introEditable">
                        <breeze-label for="intro_override" class="select-label" value="Intro"></breeze-label>
...
export default {
    name: "CibLandingCopies",
    emits: [
        "nextClicked", "backClicked",
        "subheadUpdated", "ctaUpdated",
        "introUpdated", "bodyTextUpdated",
    ],
    props: [
    ...
    mounted() {
        document.getElementById('layout_intro_text').innerHTML = "<div>Please <b>select the copy for your " + this.chooseTitle + " Landing Copy from the dropdown menus below</b>. Once you have selected your text options, you may use the edit button to make minor changes to the copy directly in the selection box. Pressing Cancel will reset the text to your original choice. Please keep in mind that font size and colors are not editable and there are character limits in place.</div>";

        const ps = this.$parent.preSelected;

        [{v:"subhead", p:"landing_subhead"}, {v:"intro", p:"landing_intro"}, {v:"bodyText", p:"landing_body"}, {v:"cta", p: "landing_cta"}].forEach(x => {
        ...
        if(!this.subhead || !a.landing_subhead.includes(this.subhead)) {
            this.subhead = a.landing_subhead[0];
        }

        if(!this.intro || !a.landing_intro.includes(this.intro)) {
            this.intro = a.landing_intro[0];
        }

        if(!this.bodyText || !a.landing_body.includes(this.bodyText)) {
            this.bodyText = a.landing_body[0];
        }

        if(!this.cta || !a.landing_cta.includes(this.cta)) {
            this.cta = a.landing_cta[0];
        }
    },
    ...
    computed: {
        formOk() {
            return this.subhead && this.cta && this.intro && this.bodyText;
        },
        ...
        subheadSelect() {
            ...
            const a = this.$parent.copies.common[this.selector].landing_subhead;
            for(let k in a) {
                list.push({
                    value: a[k],
                    label: a[k],//.strip,
        ...
        ctaSelect() {
            ...
            let list = [];
            const a = this.$parent.copies.common[this.selector].landing_cta;
            for(let k in a) {
                list.push({
                    value: a[k],
                    label: a[k],
        ...
        introSelect() {
            ...
            const a = this.$parent.copies.common[this.selector].landing_intro;
            for(let k in a) {
                list.push({
                    value: a[k],
                    label: a[k],//.strip,
        ...
        bodyTextSelect() {
            ...
            // let list = [{value: "", label: "Please Select ..."}];
            let list = [];
            const a = this.$parent.copies.common[this.selector].landing_body;
            for(let k in a) {
                list.push({
                    value: a[k],
